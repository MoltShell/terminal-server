...
- **Process must have uncaughtException/unhandledRejection handlers**: Without them, an unhandled promise rejection or exception silently crashes the Node process. systemd restarts it, but the error is lost. Fix: add `process.on('uncaughtException')` and `process.on('unhandledRejection')` handlers that log the error before exiting.
- **SIGTERM handler needed for graceful shutdown**: Without it, `systemctl restart` sends SIGTERM → Node dies immediately → WebSocket clients get no close frame → xterm.js shows "disconnected" with no explanation. Fix: `process.on('SIGTERM')` that closes all WS clients with code 1001, then shuts down the HTTP server.
- **tmux history-limit defaults to 2000 lines — can consume significant RAM**: Each tmux pane stores scrollback in RAM. With multiple panes and heavy output, default 2000-line history per pane adds up. Fix: `tmux set -g history-limit 5000` in `enableMouseModeAll()` — high enough for usability, low enough to limit RAM. The memory watchdog clears scrollback at 85% usage as an additional safety layer.
- **`/health` endpoint should include memory stats**: The basic `{ status: 'ok' }` response gives no visibility into VM resource usage. Fix: read `/proc/meminfo`, calculate RAM + swap usage, include in health response. The health checker and monitoring can use this data.
- **WebSocket `system-alert` messages for memory warnings**: The server broadcasts `{ type: 'system-alert', level: 'warning'|'critical', message }` to all connected clients when memory exceeds 80%/90%. Frontend shows a dismissable banner. Alerts deduplicate (same level re-sent only after 60s).
- **WebSocket ping/pong required to detect dead connections for idle shutdown**: Node.js `ws` library does NOT enable TCP keepalive by default. Without ping/pong, when a user closes their laptop or loses network, the TCP connection stays open indefinitely on the server. `ws.readyState` stays `OPEN`, so the heartbeat timer keeps firing on ghost connections, updating `last_activity_at` in D1, preventing the idle checker from ever suspending the VM. Fix: server pings all clients every 30s; if no pong by next ping, `ws.terminate()` kills the connection. Dead connections detected within ~1 minute → heartbeat timer cleared → VM can go idle.
